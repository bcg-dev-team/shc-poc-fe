## SHC PoC 모바일 화면 마이그레이션 가이드

SHC PoC 같은 Figma 기반 모바일 화면을 `apps/investor`에 옮길 때 일관되게 사용할 수 있는 패턴을 정리합니다.
...
- Figma → React Export된 코드는 다음 경로에 있습니다.
  - `shc-investor/src/imports/ShcUi*.tsx`
- 현재 레포에서는 재사용/참고용으로 그대로 복사되어 있습니다.
  - `apps/investor/src/imports/ShcUi*.tsx`

> **원칙**: `imports/ShcUi*` 파일은 _직접 렌더링하지 않고_, 레이아웃/텍스트/간격을 참고하는 **디자인 소스**로만 사용합니다.

#### 2-2. 신규 화면 컴포넌트 위치

- 실제 라우트에 사용되는 코드는 다음 위치에 작성합니다.
  - `apps/investor/src/components/shc/PurchaseScreen.tsx`
  - `apps/investor/src/components/shc/PurchaseCompleteScreen.tsx`
  - `apps/investor/src/components/shc/AssetsScreen.tsx`
  - `apps/investor/src/components/shc/RedeemScreen.tsx`
  - `apps/investor/src/components/shc/RedeemCompleteScreen.tsx`

### 3. 마이그레이션 단계별 패턴

#### 3-1. 화면 분석

1. `imports/ShcUi*.tsx` 원본 파일에서 **크게 3~5개의 섹션**으로 나눌 수 있는 구조를 찾습니다.
...
1. `imports/ShcUi*.tsx`에서 레이아웃/텍스트/섹션 구조를 분석한다.
2. `components/shc/*Screen.tsx`에 공통 스캐폴딩(헤더/본문/푸터)을 만든다.
...
7. 빌드 (`pnpm --filter @shc/investor build`)로 검증한다.
